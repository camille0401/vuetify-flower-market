...
import FSSnackbar from './FSSnackbar.vue'
...
const $snackbar = (options) => {
  ...
  const app = createApp(FSSnackbar, {
    ...options,
    hide() {
      // 卸载已挂载的应用实例
      app.unmount()
...
$snackbar.install = (app) => {
  // 注册全局属性，类似于 Vue2 的 Vue.prototype
  app.config.globalProperties.$snackbar = (options) => $snackbar(options).show()
}
// 定义show方法用于直接调用
$snackbar.show = (options) => $snackbar(options).show()
